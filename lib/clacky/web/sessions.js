...
const Sessions = (() => {
  ...
  return {
    ...
    _setActiveId(id) {
      _activeId = id;
      this._lastTokenModel = null; // Reset model tracking on session switch
    },
    ...
    appendTokenUsage(ev, container) {
      ...
      // Model name: show badge when the model changes (tracked per-session)
      const currentModel = ev.model || "";
      if (currentModel && this._lastTokenModel !== currentModel) {
        el.classList.add("tu-with-switch");
        const badge = document.createElement("span");
        badge.className = "tu-model-badge";
        badge.textContent = currentModel;
        el.appendChild(badge);
        this._lastTokenModel = currentModel;
      }
...
// ── Session Info Bar Model Switcher (Modal) ────────────────────────────────
(function() {
  let _sessionId = null;
  let _currentModelId = null;
  let _models = [];
  let _activeProviderIndex = -1;
  // Cache of the most recent benchmark results, keyed by model_id. Kept at
  // closure scope so the numbers survive closing & reopening the modal —
  ...
  // Open the session model switch modal on click
  document.addEventListener("click", async (e) => {
    ...
    if (modelEl) {
      ...
      _sessionId = modelEl.dataset.sessionId;
      _currentModelId = modelEl.dataset.modelId || null;
      await _openSessionModelModal();
      return;
    }
  });
  ...
  document.addEventListener("click", (e) => {
    const modal = document.getElementById("session-model-modal");
    if (!modal || modal.style.display === "none") return;

    // Close button
    if (e.target.closest("#session-model-modal-close")) {
      _closeSessionModelModal();
      return;
    }
  ...
  function _closeSessionModelModal() {
    const modal = document.getElementById("session-model-modal");
    if (!modal) return;
  ...
  async function _openSessionModelModal() {
    const modal = document.getElementById("session-model-modal");
    if (!modal) return;
    modal.style.display = "";
    document.body.style.overflow = "hidden";
    await _loadModelsAndRender();
  }

  // Group models by provider name, load from config, render provider list
  async function _loadModelsAndRender() {
    try {
      const res = await fetch("/api/config");
      const data = await res.json();
      _models = data.models || [];
      ...
      if (_models.length === 0) {
        const providerList = document.getElementById("session-model-provider-list");
        const modelList = document.getElementById("session-model-model-list");
        if (providerList) providerList.innerHTML = `<div class="session-model-picker-empty">${I18n.t ? I18n.t("settings.models.empty") : "No models configured"}</div>`;
        if (modelList) modelList.innerHTML = "";
        return;
      }

      // Find the currently active model index
      const currentIndex = Math.max(0, _models.findIndex(m => m.id === _currentModelId));
      _activeProviderIndex = currentIndex >= 0 ? currentIndex : 0;
      ...
      const providerList = document.getElementById("session-model-provider-list");
      if (providerList) {
        providerList.innerHTML = `<div class="session-model-picker-empty" style="color:var(--color-error)">${I18n.t ? I18n.t("settings.models.loadFailed") : "Error loading models"}</div>`;
      }
    }
  }
  ...
  function _providerLabel(model) {
    ...
    if (!model.base_url) return "custom";
    return model.base_url.replace(/^https?:\/\//, "").replace(/\/+$/, "");
  }
  function _renderProviderList() {
    ...
    providerList.innerHTML = _models.map((m, i) => `
      <button type="button" class="session-model-provider-item ${i === _activeProviderIndex ? "active" : ""}" data-index="${i}">
        <span>${_esc(_providerLabel(m))}</span>
        <small>${_esc(m.base_url || "")}</small>
      </button>
    `).join("");

    providerList.querySelectorAll(".session-model-provider-item").forEach(btn => {
      btn.addEventListener("click", () => {
        _activeProviderIndex = parseInt(btn.dataset.index, 10);
        _renderProviderList();
        _renderModelList();
      });
    });
  ...
  async function _renderModelList() {
    const modelList = document.getElementById("session-model-model-list");
    if (!modelList) return;
    const provider = _models[_activeProviderIndex];
    if (!provider) {
      modelList.innerHTML = `<div class="session-model-picker-empty">${I18n.t ? I18n.t("settings.models.noModels") : "No models"}</div>`;
      return;
    }

    modelList.innerHTML = `<div class="session-model-picker-empty">${I18n.t ? I18n.t("settings.models.loadingModels") : "Loading..."}</div>`;
    const models = await _fetchProviderModels(provider, _activeProviderIndex);
    if (!models.length) {
      modelList.innerHTML = `<div class="session-model-picker-empty">${I18n.t ? I18n.t("settings.models.noModels") : "No models"}</div>`;
      return;
    }

    // If the provider's configured model name is in the list, pre-select it
    const currentModelName = provider.model || "";

    modelList.innerHTML = models.map(name => {
      const isCurrent = name === currentModelName && provider.id === _currentModelId;
      return `
        <button type="button" class="session-model-model-item ${isCurrent ? "active" : ""}" data-model="${_esc(name)}" data-provider-index="${_activeProviderIndex}">
          <span class="session-model-model-name">${_esc(name)}</span>
          ${_benchCache[provider.id] ? `<span class="sib-model-latency"></span>` : ""}
        </button>
      `;
    }).join("");

    // Fill latency cells from cache if available
    if (_benchCache[provider.id]) {
      const latEl = modelList.querySelector(".sib-model-latency");
      if (latEl) _fillLatencyCell(latEl, _benchCache[provider.id]);
    }

    modelList.querySelectorAll(".session-model-model-item").forEach(btn => {
      btn.addEventListener("click", async () => {
        const modelName = btn.dataset.model;
        const idx = parseInt(btn.dataset.providerIndex, 10);
        const target = _models[idx];
        if (!target || !target.id) return;
        await _switchModel(_sessionId, target.id, modelName);
      });
    });
  }

  async function _fetchProviderModels(provider, index) {
