...
const Sessions = (() => {
  ...
  function _renderSessionItem(container, s) {
    ...
    const displayName = s.source === "channel"
      ? (s.name || "").replace(/-\d+$/, "") || _relativeTime(s.created_at)
      : (s.name || _relativeTime(s.created_at));
    ...
    // Channel platform name map — extract from session name (e.g. "feishu-1" → "Feishu")
    const _channelPlatformNames = { feishu: "Feishu", discord: "Discord", telegram: "Telegram", wecom: "WeCom", wechat: "WeChat", dingtalk: "DingTalk" };
    const _channelPlatform = (s.source === "channel")
      ? _channelPlatformNames[(s.name || "").replace(/-\d+$/, "").toLowerCase()] || (s.name || "").replace(/-\d+$/, "")
      : null;

    const badgeKey = s.source === "cron"    ? "sessions.badge.cron"
                   : s.source === "web"     ? "sessions.badge.web"
                   : s.source === "setup"   ? "sessions.badge.setup"
                   : null;
    const badgeText = _channelPlatform || (badgeKey ? I18n.t(badgeKey) : "");
    const badgeHtml = badgeText
      ? `<span class="session-badge session-badge--${s.source}">${badgeText}</span>`
      : "";
    ...
    // Running sessions use CSS animation instead of a dot.
    const isRunning = s.status === "running";
    const dotHtml = (!isRunning && s.status && s.status !== "idle")
      ? `<span class="session-dot dot-${s.status}"></span>`
      : "";

    // Add running class for animated background
    if (isRunning) el.classList.add("session-item--running");

    el.innerHTML = `<div class="session-body">
      <div class="session-name">${dotHtml}<span class="session-name__text">${escapeHtml(displayName)}</span>${pinIcon}</div>
      <div class="session-footer">
        ${(badgeHtml||codingBadgeHtml) ? `<div class="session-badges">${badgeHtml}${codingBadgeHtml}</div>` : ''}
  ...
  return {
    ...
    async create(agentProfile = "general") {
      ...
      const res  = await fetch("/api/sessions", {
        method:  "POST",
        headers: { "Content-Type": "application/json" },
        body:    JSON.stringify({ name, agent_profile: agentProfile, source: "web" })
      });
      const data = await res.json();
