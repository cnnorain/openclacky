...
const Plugins = (() => {
  ...
  async function _showPluginSettings(plugin) {
    ...
    // Fetch current config and schema
    let config = {};
    let schema = {};
    try {
    ...
    // Helper to create a field HTML from schema
    const createSchemaFieldHtml = (key, fieldSchema, value) => {
      const label = fieldSchema.label || key;
      const desc = fieldSchema.description || "";
      const placeholder = fieldSchema.placeholder || "";
      const type = fieldSchema.type || "string";
      const requiredMark = fieldSchema.required ? `<span class="ps-required">*</span>` : "";
      const defaultValue = fieldSchema.default || "";
      
      // Use current value, or default if not set
      let currentValue = value !== undefined ? value : defaultValue;
      
      // Build label with description inline
      const labelHtml = desc 
        ? `<label class="ps-field-label">${requiredMark}${escapeHtml(label)}：<span class="ps-field-desc-inline">${escapeHtml(desc)}</span></label>`
        : `<label class="ps-field-label">${requiredMark}${escapeHtml(label)}</label>`;
      
      // For arrays, convert to newline-separated string for editing
      if (type === "array") {
        if (Array.isArray(currentValue)) {
          currentValue = currentValue.join("\n");
        }
        return `
          <div class="ps-field" data-field-key="${escapeHtml(key)}" data-type="array">
            <div class="ps-field-header">
              ${labelHtml}
            </div>
            <textarea class="ps-field-input ps-textarea" data-key="${escapeHtml(key)}" placeholder="${escapeHtml(placeholder)}">${escapeHtml(String(currentValue))}</textarea>
          </div>
        `;
      }
      
      // For objects or long text, use textarea
      const valueStr = typeof currentValue === "object" ? JSON.stringify(currentValue, null, 2) : String(currentValue || "");
      const isMultiline = type === "object" || type === "text" || valueStr.includes("\n") || valueStr.length > 80;
      
      if (isMultiline) {
        return `
          <div class="ps-field" data-field-key="${escapeHtml(key)}" data-type="${escapeHtml(type)}">
            <div class="ps-field-header">
              ${labelHtml}
            </div>
            <textarea class="ps-field-input ps-textarea" data-key="${escapeHtml(key)}" placeholder="${escapeHtml(placeholder)}">${escapeHtml(valueStr)}</textarea>
          </div>
        `;
      }
      ...
      return `
        <div class="ps-field" data-field-key="${escapeHtml(key)}" data-type="${escapeHtml(type)}">
          ...
          <input type="text" class="ps-field-input" data-key="${escapeHtml(key)}" value="${escapeHtml(valueStr)}" placeholder="${escapeHtml(placeholder)}">
        </div>
      `;
    };

    // Helper to create a field HTML for custom keys (not in schema)
    const createCustomFieldHtml = (key, value) => {
      const valueStr = typeof value === "object" ? JSON.stringify(value, null, 2) : String(value);
      const isMultiline = typeof value === "object" || valueStr.includes("\n") || valueStr.length > 50;
      if (isMultiline) {
        return `
          <div class="ps-field ps-custom-field" data-field-key="${escapeHtml(key)}">
            <div class="ps-field-header">
    ...
    if (schemaKeys.length > 0) {
      ...
      fieldsHtml = schemaKeys.map(key => createSchemaFieldHtml(key, schema[key], config[key])).join("");
      
      // Add any custom config keys not in schema
      const customKeys = configKeys.filter(k => !schemaKeys.includes(k) && k !== "enabled");
      if (customKeys.length > 0) {
        fieldsHtml += `<div class="ps-custom-section"><div class="ps-section-label">${I18n.t("plugins.settings.custom") || "Custom"}</div>`;
        fieldsHtml += customKeys.map(key => createCustomFieldHtml(key, config[key])).join("");
        fieldsHtml += `</div>`;
      }
    } else if (configKeys.length > 0) {
      // No schema, show existing config as custom fields
      fieldsHtml = configKeys.filter(k => k !== "enabled").map(key => createCustomFieldHtml(key, config[key])).join("");
    }
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      modal.querySelectorAll(".ps-field-input").forEach(input => {
        ...
        const field = input.closest(".ps-field");
        const fieldType = field?.dataset.type;
        let value = input.value;
        
        // Handle array type: split by newlines
        if (fieldType === "array") {
