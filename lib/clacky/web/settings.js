...
const Settings = (() => {
  ...
  let _models = [];
  let _selectedDefaultModelId = null;
  // Provider presets loaded from server
  let _providers = [];
  let _remoteModelCache = new Map();
  let _remoteModelRequestSeq = 0;
  ...
  async function _load() {
    ...
    try {
      ...
      _selectedDefaultModelId = configData.current_id || (_models.find(m => m.type === "default") || _models[0] || {}).id || null;
      _providers = providerData.providers || [];
      _renderDefaultModelSetting();
      _renderCards();
    } catch (e) {
  ...
  function _renderCards() {
    ...
    // Group models by base_url so each provider shows all its model names
    const groups = [];
    const groupMap = {};
    _models.forEach((m, i) => {
      const key = (m.base_url || "").trim().replace(/\/+$/, "");
      if (!groupMap[key]) {
        groupMap[key] = { models: [], indices: [] };
    ...
    groups.forEach(group => {
      _renderCard(container, group.models, group.indices);
    });
  }

  function _renderDefaultModelSetting() {
    const el = document.getElementById("default-model-setting");
    if (!el) return;
    ...
    el.style.display = "";
    const current = _models.find(m => m.id === _selectedDefaultModelId) || _models.find(m => m.type === "default") || _models[0];
    const label = current ? `${current.provider_name || _getProviderName(current)} · ${current.model || I18n.t("settings.models.unnamed")}` : I18n.t("settings.models.defaultSelector.choose");

    el.innerHTML = `
      <div>
        <div class="default-model-setting-label">${I18n.t("settings.models.defaultSelector.label")}</div>
        <div class="default-model-setting-hint">${I18n.t("settings.models.defaultSelector.hint")}</div>
      </div>
      <button id="default-model-select" class="default-model-select" type="button">
        <span>${_esc(label)}</span>
        <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
      </button>
    `;

    el.querySelector("#default-model-select").addEventListener("click", _openDefaultModelModal);
  }
  ...
  function _renderCard(container, models, indices) {
    // models: array of model objects sharing the same base_url
    // indices: corresponding indices in _models
    const first = models[0];
    const isDefault = models.some(m => m.type === "default");
    const isLite    = models.some(m => m.type === "lite");
    const providerName = first.provider_name || _getProviderName(first);
    const displayName = providerName || I18n.t("settings.models.provider.custom");

    const card = document.createElement("div");
    card.className = "model-card-grid" + (isDefault ? " model-card-grid-default" : "");
    card.dataset.index = indices.join(",");
    ...
    card.innerHTML = `
      <div class="model-card-grid-info">
        ...
        <div class="model-card-grid-provider">${_esc(first.base_url || "")}</div>
        <div class="model-card-grid-status">
          ${indices.map(i => `<span class="model-test-result" data-index="${i}"></span>`).join("")}
        </div>
      </div>
      <div class="model-card-grid-actions">
        <button class="btn-card-grid-action" data-index="${indices[0]}" data-action="test" title="${I18n.t("settings.models.btn.test")}">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          <span>${I18n.t("settings.models.btn.test")}</span>
        </button>
        <button class="btn-card-grid-action" data-index="${indices[0]}" data-action="edit" title="${I18n.t("settings.models.btn.edit")}">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          <span>${I18n.t("settings.models.btn.edit")}</span>
        </button>
        ${_models.length > 1 ? `<button class="btn-card-grid-action btn-card-grid-action-danger" data-index="${indices[0]}" data-action="delete" title="${I18n.t("settings.models.btn.delete")}">
    ...
    _bindCompactCardEvents(card, indices[0]);
  }
  ...
  function _openModal(index = -1) {
    ...
    if (index >= 0 && _models[index]) {
      ...
      document.getElementById("model-modal-provider-name").value = model.provider_name || "";
      ...
      document.getElementById("model-modal-model-field").style.display = index >= 0 ? "" : "none";
      document.getElementById("model-modal-provider-name-field").style.display = model.provider_name ? "" : "none";

      // Set provider dropdown value
      const providerName = model.provider_name || _getProviderName(model);
      ...
      // Load and display model list
      setTimeout(() => _updateModalModelDropdown(), 100);
    } else {
      // Add mode
      titleEl.textContent = I18n.t("settings.models.modal.add");
      document.getElementById("model-modal-model").value = "";
      document.getElementById("model-modal-provider-name").value = "";
      document.getElementById("model-modal-baseurl").value = "";
      document.getElementById("model-modal-apikey").value = "";
      document.getElementById("model-modal-default-field").style.display = "none";
      document.getElementById("model-modal-model-field").style.display = "none";
      document.getElementById("model-modal-provider-name-field").style.display = "none";
  ...
  function _populateModalProviderDropdown() {
    ...
    dropdown.querySelectorAll(".custom-select-option").forEach(option => {
      option.addEventListener("click", (e) => {
        ...
        // Show/hide custom provider name field
        const providerNameField = document.getElementById("model-modal-provider-name-field");
        if (providerNameField) providerNameField.style.display = value === "custom" ? "" : "none";
        ...
        if (value && value !== "custom") {
          ...
          if (preset) {
            ...
            document.getElementById("model-modal-provider-name").value = preset.name || "";
  ...
  async function _saveModalModel() {
    ...
    let model = document.getElementById("model-modal-model").value.trim();
    const provider_name = document.getElementById("model-modal-provider-name").value.trim();
    ...
    if (shouldTest) {
      ...
      try {
        const testRes = await fetch("/api/config/test", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ base_url, api_key, index })
        });
        const testData = await testRes.json();
    ...
    const payload = { model, provider_name, base_url, anthropic_format: false };
    payload.type = existing.type || null;
  ...
  function _initModal() {
    ...
    const defaultModal = document.getElementById("default-model-modal");
    document.getElementById("default-model-modal-close")?.addEventListener("click", _closeDefaultModelModal);
    defaultModal?.addEventListener("click", (e) => {
      if (e.target.id === "default-model-modal") _closeDefaultModelModal();
    });
    ...
    document.addEventListener("keydown", (e) => {
      const modelModalOpen = document.getElementById("model-edit-modal").style.display !== "none";
      const defaultModalOpen = document.getElementById("default-model-modal")?.style.display !== "none";
      if (e.key === "Escape" && modelModalOpen) {
        _closeModal();
      } else if (e.key === "Escape" && defaultModalOpen) {
        _closeDefaultModelModal();
      }
    });
    ...
    document.addEventListener("langchange", () => {
      _renderDefaultModelSetting();
      if (document.getElementById("default-model-modal")?.style.display !== "none") {
        _renderDefaultModelProviderList();
      }
    });
    ...
    document.getElementById("model-modal-apikey").addEventListener("blur", () => {
      _remoteModelCache.clear();
    });
  ...
  async function _updateModalModelDropdown() {
    const modelList = document.getElementById("model-modal-model-list");
    const modelInput = document.getElementById("model-modal-model");
    if (!modelList || !modelInput) return;

    const baseUrl = document.getElementById("model-modal-baseurl").value.trim();
    const apiKey = document.getElementById("model-modal-apikey").value.trim();
    const index = parseInt(document.getElementById("model-modal-index").value, 10);
    const provider = _getModalCurrentProvider();
    let models = provider?.models || [];

    if (baseUrl && apiKey) {
      const cacheKey = `${baseUrl}|${apiKey.includes("****") ? "masked" : apiKey.slice(0, 12)}`;
      if (_remoteModelCache.has(cacheKey)) {
        models = _remoteModelCache.get(cacheKey);
      } else {
        const seq = ++_remoteModelRequestSeq;
        modelList.innerHTML = `<div class="model-list-empty">${I18n.t("settings.models.loading") || "Loading models..."}</div>`;
        try {
          const res = await fetch("/api/config/models/list", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ base_url: baseUrl, api_key: apiKey, index })
          });
          const data = await res.json();
          if (seq !== _remoteModelRequestSeq) return;
          if (data.ok && Array.isArray(data.models) && data.models.length > 0) {
            models = data.models;
            _remoteModelCache.set(cacheKey, models);
          }
        } catch (_) {
          // Keep provider presets as a fallback if the remote list cannot be loaded.
        }
      }
    }

    if (models.length === 0) {
      modelList.innerHTML = `<div class="model-list-empty">${I18n.t("settings.models.noModels") || "No models available"}</div>`;
      return;
    }

    const currentValue = modelInput.value;
    modelList.innerHTML = models.map(m =>
      `<div class="model-list-item${m === currentValue ? " selected" : ""}" data-value="${_esc(m)}">${_esc(m)}</div>`
    ).join("");
    ...
    modelList.querySelectorAll(".model-list-item").forEach(item => {
      item.addEventListener("click", (e) => {
        e.stopPropagation();
        // Deselect previous
        modelList.querySelector(".model-list-item.selected")?.classList.remove("selected");
        // Select clicked
  ...
  function _readCard(index) {
    ...
    return {
      ...
      provider_name:    card.querySelector(`[data-key="provider_name"]`)?.value.trim() || "",
  ...
  function _openDefaultModelModal() {
    const modal = document.getElementById("default-model-modal");
    if (!modal) return;
    modal.style.display = "";
    document.body.style.overflow = "hidden";
    _renderDefaultModelProviderList();
  ...
  function _closeDefaultModelModal() {
    const modal = document.getElementById("default-model-modal");
    if (!modal) return;
    modal.style.display = "none";
    document.body.style.overflow = "";
  ...
  function _providerLabel(model) {
    return model.provider_name || _getProviderName(model);
  }

  function _renderDefaultModelProviderList(selectedIndex = 0) {
    const providerList = document.getElementById("default-model-provider-list");
    if (!providerList) return;

    if (_models.length === 0) {
      providerList.innerHTML = `<div class="default-model-picker-empty">${I18n.t("settings.models.empty")}</div>`;
      _renderDefaultModelList(-1);
      return;
    }

    const currentIndex = Math.max(0, _models.findIndex(m => m.id === _selectedDefaultModelId));
    const activeIndex = selectedIndex >= 0 ? selectedIndex : currentIndex;
    providerList.innerHTML = _models.map((m, i) => `
      <button type="button" class="default-model-provider-item ${i === activeIndex ? "active" : ""}" data-index="${i}">
        <span>${_esc(_providerLabel(m))}</span>
        <small>${_esc(m.base_url || "")}</small>
      </button>
    `).join("");

    providerList.querySelectorAll(".default-model-provider-item").forEach(btn => {
      btn.addEventListener("click", () => {
        const idx = parseInt(btn.dataset.index, 10);
        _renderDefaultModelProviderList(idx);
      });
    });

    _renderDefaultModelList(activeIndex);
  }

  async function _renderDefaultModelList(index) {
    const modelList = document.getElementById("default-model-model-list");
    if (!modelList) return;
    const provider = _models[index];
    if (!provider) {
      modelList.innerHTML = `<div class="default-model-picker-empty">${I18n.t("settings.models.noModels")}</div>`;
      return;
    }

    modelList.innerHTML = `<div class="default-model-picker-empty">${I18n.t("settings.models.loadingModels")}</div>`;
    const models = await _fetchModelsForProvider(provider, index);
    if (!models.length) {
      modelList.innerHTML = `<div class="default-model-picker-empty">${I18n.t("settings.models.noModels")}</div>`;
      return;
    }
    ...
    modelList.innerHTML = models.map(name => `
      <button type="button" class="default-model-model-item ${name === provider.model ? "active" : ""}" data-model="${_esc(name)}">
        ${_esc(name)}
      </button>
    `).join("");

    modelList.querySelectorAll(".default-model-model-item").forEach(btn => {
      btn.addEventListener("click", () => _selectDefaultRemoteModel(index, btn.dataset.model));
    });
  }

  async function _fetchModelsForProvider(provider, index) {
    const baseUrl = (provider.base_url || "").trim();
    const apiKey = (provider.api_key_masked || "").trim();
    if (!baseUrl) return provider.model ? [provider.model] : [];
    const cacheKey = `default|${provider.id || index}|${baseUrl}`;
    if (_remoteModelCache.has(cacheKey)) return _remoteModelCache.get(cacheKey);
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok && Array.isArray(data.models)) models = data.models;
  ...
  async function _selectDefaultRemoteModel(index, modelName) {
    const target = _models[index];
    if (!target || !target.id || !modelName) {
      alert(I18n.t("settings.models.setDefaultFailed"));
      return;
    }

    try {
      const updateRes = await fetch(`/api/config/models/${encodeURIComponent(target.id)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: modelName })
      });
      const updateData = await updateRes.json();
      if (!updateData.ok) throw new Error(updateData.error || I18n.t("settings.models.saveFailed"));
      await _setAsDefault(index);
      _closeDefaultModelModal();
    } catch (e) {
  ...
  async function _setAsDefault(index) {
    ...
    _selectedDefaultModelId = target.id;
  ...
  async function _loadCurrentVersion() {
    try {
      const res = await fetch("/api/version");
      const data = await res.json();
      const currentVersionEl = document.getElementById("update-current-version");
      if (currentVersionEl && data.current) {
        currentVersionEl.textContent = data.current;
      }
    } catch (e) {
  ...
  async function _loadUpdateHistory() {
    try {
      ...
      if (data.history && data.history.length > 0) {
        historyContainer.innerHTML = data.history.map(item => `
          <div class="update-history-item">
            <div class="update-history-version">${_esc(item.version)}</div>
            <div class="update-history-date">${_esc(item.timestamp)}</div>
            <div class="update-history-status success">
              ✓
            </div>
          </div>
        `).join("");
      } else {
        historyContainer.innerHTML = `<div class="update-history-empty">${I18n.t("settings.update.noHistory")}</div>`;
      }
    } catch (e) {
  ...
  async function _loadMergeConflicts() {
    try {
      const res = await fetch("/api/upgrade/conflicts");
      const data = await res.json();
      const conflictsContainer = document.getElementById("update-conflicts");
      if (!conflictsContainer) return;

      if (data.conflicts && data.conflicts.length > 0) {
        conflictsContainer.innerHTML = `
          <div class="update-conflicts-count">${I18n.t("settings.update.conflictsFound", { count: data.conflicts.length })}</div>
          ${data.conflicts.map(conflict => `
            <div class="update-conflict-item">
              <div class="conflict-file">${_esc(conflict.file)}</div>
              <div class="conflict-type">${_esc(conflict.type)}</div>
              <div class="conflict-description">${_esc(conflict.preview || "")}</div>
            </div>
          `).join("")}
        `;
      } else {
        conflictsContainer.innerHTML = `<div class="update-conflicts-empty">${I18n.t("settings.update.noConflicts")}</div>`;
      }
    } catch (e) {
      console.error("Failed to load merge conflicts:", e);
    }
  }

  async function _loadUpdateSettings() {
    try {
      const res = await fetch("/api/upgrade/settings");
      const data = await res.json();

      const autoCheck = document.getElementById("update-auto-check");
      const autoBackup = document.getElementById("update-auto-backup");
      const mergeNotify = document.getElementById("update-merge-notify");

      if (autoCheck) autoCheck.checked = data.auto_check_updates !== false;
      if (autoBackup) autoBackup.checked = data.backup_before_update !== false;
      if (mergeNotify) mergeNotify.checked = data.notify_conflicts !== false;
    } catch (e) {
  ...
  async function _checkForUpdates() {
    ...
    btn.disabled = true;
    btn.textContent = I18n.t("settings.update.checking");
    ...
    try {
      ...
      if (latestVersionEl && data.latest_version) {
      ...
      if (statusEl) {
        if (data.needs_update) {
          statusEl.innerHTML = `
            <span class="update-status-icon">⬆</span>
            <span class="update-status-text">${I18n.t("settings.update.updateAvailable")}</span>
          `;
          statusEl.className = "update-status update-available";
        } else {
          statusEl.innerHTML = `
            <span class="update-status-icon">✓</span>
            <span class="update-status-text">${I18n.t("settings.update.upToDate")}</span>
          `;
          statusEl.className = "update-status update-up-to-date";
        }
      }
    } catch (e) {
      ...
      btn.disabled = false;
      btn.textContent = I18n.t("settings.update.checkForUpdate");
    }
  }

  async function _updateNow() {
    const btn = document.getElementById("btn-update-now");
    if (!btn) return;

    btn.disabled = true;
    btn.textContent = I18n.t("settings.update.updating");
    ...
    try {
      ...
      if (data.ok) {
        alert(I18n.t("settings.update.success"));
        _loadUpdateInfo();
      } else {
        alert(I18n.t("settings.update.failed") + ": " + (data.error || ""));
      }
    } catch (e) {
      console.error("Failed to update:", e);
      alert(I18n.t("settings.update.failed") + ": " + e.message);
    } finally {
      btn.disabled = false;
      btn.textContent = I18n.t("settings.update.updateNow");
    }
  }

  async function _saveUpdateSettings() {
    const autoCheck = document.getElementById("update-auto-check");
    const autoBackup = document.getElementById("update-auto-backup");
    const mergeNotify = document.getElementById("update-merge-notify");

    const settings = {
      auto_check_updates: autoCheck ? autoCheck.checked : true,
      backup_before_update: autoBackup ? autoBackup.checked : true,
      notify_conflicts: mergeNotify ? mergeNotify.checked : true
    };

    try {
      await fetch("/api/upgrade/settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function _clearOldSnapshots() {
    if (!confirm(I18n.t("settings.update.confirmClearSnapshots"))) return;

    try {
      const res = await fetch("/api/upgrade/snapshots", { method: "DELETE" });
      const data = await res.json();

      if (data.ok) {
        alert(I18n.t("settings.update.snapshotCleared"));
      }
    } catch (e) {
  ...
  function _initUpdateTab() {
    // Check for updates button
    const checkUpdateBtn = document.getElementById("btn-check-update");
    if (checkUpdateBtn) {
      checkUpdateBtn.addEventListener("click", _checkForUpdates);
    }

    // Update now button
    const updateNowBtn = document.getElementById("btn-update-now");
    if (updateNowBtn) {
      updateNowBtn.addEventListener("click", _updateNow);
    }

    // View snapshots button
    const viewSnapshotsBtn = document.getElementById("btn-view-snapshots");
    if (viewSnapshotsBtn) {
      viewSnapshotsBtn.addEventListener("click", () => {
        // TODO: Implement snapshots viewer
        alert("Snapshots viewer coming soon");
      });
    }
    ...
    if (clearSnapshotsBtn) {
      clearSnapshotsBtn.addEventListener("click", _clearOldSnapshots);
    }
    ...
    if (autoCheck) autoCheck.addEventListener("change", _saveUpdateSettings);
    if (autoBackup) autoBackup.addEventListener("change", _saveUpdateSettings);
    if (mergeNotify) mergeNotify.addEventListener("change", _saveUpdateSettings);
  }
  ...
  function _initTabs() {
    ...
    tabs.forEach(tab => {
      tab.addEventListener("click", () => {
        ...
        // Load update info when update tab is selected
        if (targetTab === "update") {
          _loadUpdateInfo();
        }
      });
