...
const Settings = (() => {
  ...
  function _renderCard(container, model, index) {
    ...
    const displayName = model.name || model.model || I18n.t("settings.models.unnamed");

    const card = document.createElement("div");
    card.className = "model-card-grid";
  ...
  function _openModal(index = -1) {
    ...
    if (index >= 0 && _models[index]) {
      ...
      document.getElementById("model-modal-name").value = model.name || "";
    ...
    document.getElementById("model-modal-name").focus();
  }
  ...
  async function _saveModalModel() {
    ...
    const payload = { name, model, base_url, anthropic_format: false };
    if (isNew) {
