...
const Billing = (() => {
  ...
  /** Set up delegated event handlers on the container so they survive innerHTML swaps. */
  function _bindDelegatedEvents() {
    const container = document.getElementById("billing-content");
    if (!container) return;

    // Remove old listener to avoid duplicates, then attach
    container.removeEventListener("change", _onPeriodChange);
    container.removeEventListener("click", _onContainerClick);
    container.addEventListener("change", _onPeriodChange);
    container.addEventListener("click", _onContainerClick);
  }

  function _onPeriodChange(e) {
    const select = e.target.closest("#billing-period-select");
    if (!select) return;
  ...
  function _onContainerClick(e) {
    const btn = e.target.closest("#billing-clear-btn");
    if (!btn) return;

    // Confirmation dialog
    if (!confirm(I18n.t("billing.clearConfirm") || "Delete all billing data? This cannot be undone.")) {
      return;
    }

    fetch("/api/billing/clear", { method: "DELETE" })
      ...
      .catch(function(err) {
        console.error("Clear billing failed:", err);
      });
  }
    ...
    container.innerHTML = `
      <div class="billing-header">
        ...
        <div class="billing-header-controls">
          <button id="billing-clear-btn" class="billing-clear-btn">${I18n.t("billing.clear") || "× Clear"}</button>
