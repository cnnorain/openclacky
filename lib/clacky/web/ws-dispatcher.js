...
(function() {
  ...
  //
  // Strategy: when a phase_start arrives, we rename the live <div id="messages">
  // to "messages-outer", build a foldable card inside it, and give the card's
  // body the id "messages". Every Sessions.append* call inside the phase then
  // writes into the card without any change to sessions.js. On phase_end (or
  // any safety-net trigger) we restore the ids.
  const _phaseStack = []; // [{ id, kind, card, body, headerText, count }]
  ...
  function _beginPhase(ev) {
    ...
    const outer = document.getElementById("messages");
    if (!outer) return;
    ...
    // ID swap — sessions.js append* now writes into the card body
    outer.id = "messages-outer";
    body.id = "messages";
    ...
    // Auto-scroll to show the new phase card
  ...
  function _endPhase(phaseId, summary) {
    ...
    if (idx === -1) return;
    // Close everything from idx upwards (defensive against missed nested ends)
    while (_phaseStack.length > idx) {
      const phase = _phaseStack.pop();
      _restorePhaseDom(phase, { summary: phase.id === phaseId ? summary : null });
    }
  }

  function _restorePhaseDom(phase, { summary, incomplete } = {}) {
    const body = phase.body;
    const outer = document.getElementById("messages-outer");
    if (body && body.id === "messages") body.id = "";
    if (outer) outer.id = "messages";

    // Update summary line with final state
  ...
  function _closeAllPhases(reason) {
    while (_phaseStack.length > 0) {
      const phase = _phaseStack.pop();
      _restorePhaseDom(phase, { incomplete: reason === "incomplete" });
    }
  }

  // Expose for session-switch / disconnect cleanup
